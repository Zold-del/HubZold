...
app.get('/api/users', authenticateToken, (req, res) => {
    try {
        // Retourner tous les comptes enregistrés sauf l'utilisateur actuel
        const otherAccounts = accounts.filter(account => account.id !== req.user.id);
        
        // Transformer en format attendu par le client
        const users = otherAccounts.map(account => ({
            id: account.id,
            username: account.username,
            avatar: account.avatar,
            email: account.email,
            lastSeen: new Date() // Tous considérés comme actifs pour l'instant
        }));
...
app.post('/api/messages', authenticateToken, (req, res) => {
    ...
    const senderId = req.user.id; // Utiliser l'ID du token JWT
    ...
    try {
        ...
        console.log(`💬 Message envoyé: ${sender.username} → ${receiver.username}: "${content.substring(0, 50)}${content.length > 50 ? '...' : ''}"`);
        
        res.json({ 
            success: true,
            message: message
        });
    } catch (error) {        console.error('Erreur lors de la création du message:', error);
        res.status(500).json({ error: 'Erreur serveur' });
    }
});

app.get('/api/messages/:userId1/:userId2', authenticateToken, (req, res) => {
    ...
    try {
        ...
        // Trier par timestamp
        enrichedConversation.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
